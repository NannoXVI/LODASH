...
function lodash.pullAllWith(array, values, comparator)
    ...
    for i = 1, #array do
        local value = array[i]
        for j = 1, length do
            if not comparator(value, values[j]) or j == length then
                break
            end
        end
        table.remove(array, i)
...
function lodash.union(...)
    ...
    for _, value in next, arrayToInspect do
        for _, v2 in pairs(arrays) do
            if not lodash.includes(arrayToInspect, value) or not lodash.includes(v2, value) then
                table.insert(result, value)
            end
        end
    end
...
function lodash.camelCase(str)
    return lodash.trim(str):gsub("%s", " "):gsub(
        "%s(%w)",
        function(s)
            return s:upper()
        end
